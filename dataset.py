...
class TrainDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        ...
        label = get_label(self.gt_files[i], i, height, width)
...
def get_label(gt_box_file, height, width):
    label = np.zeros((height, width, 1)).astype((np.int32))
...
def get_train_dataloaders(batch_size,augment=False,shuffle=False,train_size=0.8):
    ...
    train_dataloder = utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=shuffle)
...
def get_Val_dataloaders(batch_size,shuffle=False,train_size=0.8):
    ...
    val_dataloaders = utils.data.DataLoader(val_dataset,batch_size=batch_size,shuffle=shuffle)
